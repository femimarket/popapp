use pop::{
    contract::interface::PopInterface,
    msg::{
        ConfigResponse, TradeResponse, PopExecuteMsgFns, PopInstantiateMsg, PopQueryMsgFns,
    },
    PopError, POP_NAMESPACE,
};
...
use cw_orch::{anyhow, daemon, prelude::*};
use cw_orch::tokio::runtime::Runtime;
use pop::state::{OrderType, Trade};

struct TestEnv<Env: CwEnv> {
    abs: AbstractClient<Env>,
    app: Application<Env, PopInterface<Env>>,
}
...
impl TestEnv<MockBech32> {
    ...
    fn setup() -> anyhow::Result<TestEnv<MockBech32>> {
        ...
        publisher.publish_app::<PopInterface<_>>()?;

        let app = publisher
            .account()
            .install_app::<PopInterface<_>>(&PopInstantiateMsg { count: 0 }, &[])?;
...
impl TestEnv<Daemon> {
    fn setup() -> anyhow::Result<TestEnv<Daemon>> {
        // Setup
        let rt = Runtime::new()?;
        let chain = daemon::DaemonBuilder(network)
            .handle(rt.handle())
            .build()?;
        ...
        let abstract_client: AbstractClient<Daemon> = abstract_client::AbstractClient(chain.clone())?;
...
fn successful_increment() -> anyhow::Result<()> {
    ...
    app.increment()?;
    let count: TradeResponse = app.count()?;
...
fn failed_reset() -> anyhow::Result<()> {
    ...
    let err: PopError = app
        .call_as(&Addr::unchecked("NotAdmin"))
        .reset(9)
        .unwrap_err()
        .downcast()
        .unwrap();
    assert_eq!(err, PopError::Admin(AdminError::NotAdmin {}));
    Ok(())
}
...
fn update_config() -> anyhow::Result<()> {
    ...
    app.update_config()?;
    let config = app.config()?;
    let expected_response = pop::msg::ConfigResponse {};

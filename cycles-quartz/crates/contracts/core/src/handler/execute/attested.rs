...
use cosmwasm_std::{to_json_binary, Deps, DepsMut, Env, MessageInfo, QueryRequest, Response, StdError, StdResult, WasmQuery};
    ...
    A: Handler + HasUserData + Attestation,
{
    fn handle(
        ...
        format!("{:?} and {:?}",msg.user_data(),attestation.user_data());
        if msg.user_data() != attestation.user_data() {


            return Err(Error::Std(StdError::generic_err(format!("{:?} and {:?}",msg.user_data(),attestation.user_data()))));
        }

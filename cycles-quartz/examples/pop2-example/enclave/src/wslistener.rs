...
use transfers_contract::msg::execute::TradeRequest;
use transfers_contract::msg::QueryMsg::{GetTradeRequests, GetTradeState};
use crate::{
...
use crate::transfers_server::UpdateTradeRequestMessage;
    ...
    A::RawAttestation: for<'de> Deserialize<'de>,
{
    ...
    // Query contract state
    let requests: Vec<TradeRequest> = cw_client
        .query_smart(contract, json!(GetTradeRequests {}))
        .await
        .map_err(|e| anyhow!("Problem querying contract state: {}", e))?;

    let state: HexBinary = cw_client
        .query_smart(contract, json!(GetTradeState {}))
        .await
        .map_err(|e| anyhow!("Problem querying contract state: {}", e))?;

    // Request body contents
    let update_contents = UpdateTradeRequestMessage { state, requests };
    ...
    let prover_config = TmProverConfig {
        ...
        verbose: "1".parse()?, // TODO: both tm-prover and cli define the same Verbosity struct. Need to define this once and import
        contract_address: contract.clone(),
        storage_key: "trade_requests".to_string(),
        chain_id: ws_config.chain_id.to_string(),
        ..Default::default()
    };

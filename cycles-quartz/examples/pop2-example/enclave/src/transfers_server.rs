...
use transfers_contract::msg::execute::{ClearTextTransferRequestMsg, Request as TransfersRequest, TradeRequest};
...
#[derive(Clone, Debug, Serialize, Deserialize, Default)]
pub struct UpdateTradeRequestMessage {
    pub state: HexBinary,
        ...
        let msg = if let Ok(message) = message {
            ...
            for req in message.requests {
                match req {
                    ...
                    // TransfersRequest::Trade(ciphertext) => {
                    //     // Decrypt transfer ciphertext into cleartext struct (acquires lock on enclave sk to do so)
                    //     let transfer = {
                    //         let sk_lock = self
                    //             .sk
            ...
            let mut state = {
                if message.state.len() == 1 && message.state[0] == 0 {
                    ...
                    decrypt_trade(sk, &message.state)?
                }
            };
...
fn decrypt_trade(
    sk: &SigningKey,
    ciphertext: &HexBinary,
) -> TonicResult<Vec<DecryptedTrade>> {
    let o =
        decrypt(&sk.to_bytes(), ciphertext).map_err(|e| Status::invalid_argument(e.to_string()))?;
    serde_json::from_slice(&o)
        .map_err(|e| Status::internal(format!("Could not deserialize transfer {}", e)))

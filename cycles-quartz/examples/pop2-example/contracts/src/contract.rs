...
pub fn instantiate(
    ...
    let state: HexBinary = HexBinary::from_hex("")?;
    TRADE_STATE.save(deps.storage, &state)?;
...
pub fn execute(
    ...
    match msg {
        ...
        ExecuteMsg::TradeRequest(msg) => trade_request(deps, env, info, msg),
...
pub fn query(deps: Deps, _env: Env, msg: QueryMsg) -> StdResult<Binary> {
    match msg {
        ...
        QueryMsg::GetTradeRequests {} => to_json_binary(&query::get_trade_requests(deps)?),
        QueryMsg::GetState {} => to_json_binary(&query::get_state(deps)?),
        QueryMsg::GetTradeState {} => to_json_binary(&query::get_trade_state(deps)?),
    }
}
mod query {
    use crate::msg::execute::TradeRequest;
    use crate::state::TRADE_REQUESTS;
    use super::*;
    ...
    pub fn get_trade_requests(deps: Deps) -> StdResult<Vec<TradeRequest>> {
        Ok(TRADE_REQUESTS.load(deps.storage)?)
    }
    ...
    pub fn get_trade_state(deps: Deps) -> StdResult<HexBinary> {
        Ok(TRADE_STATE.load(deps.storage)?)
    }
}
